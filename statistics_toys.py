...
def upperLimit ( Nobs, Nexp, sigmaexp, deltas_rel, lumi, alpha=.05, toys=20000 ):
    ...
    #print "ret=",ret
    computer = UpperLimitComputer ( deltas_rel, toys, lumi, 1.-alpha )
    ret = computer.compute ( Nobs, Nexp, sigmaexp, 5. )
    #print "ret=",ret
    return ret

class UpperLimitComputer:
    def __init__ ( self, deltas_rel, numberoftoys, lumi, cl=.95):
        """
    ...
    def f( self, sig ):
        ...
        cls = CLs ( self.nev, self.xbg, self.sbg, self.deltas_rel, sig, self.currentNToys ) - self.cl
...
def CLs(NumObserved, ExpectedBG, BGError, deltas_rel, SigHypothesis, NumToyExperiments):
    ...
    ExpectedBGandS = scipy.stats.norm.rvs( loc=ExpectedBG + SigHypothesis, 
                                        scale=sqrt(BGError**2 + (deltas_rel*SigHypothesis)**2),
                                        size=NumToyExperiments )
    ExpectedBGandS = [value for value in ExpectedBGandS if value > 0]
