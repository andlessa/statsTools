...
class NllEvalType(Enum):
    ...
    def init ( cls, evaluationType : Union[str,bool] ):
        ...
        if evaluationType in [ "false", "observed", "obs" ]:
            return cls.observed
        raise ValueError ( f"NllEvalType {evaluationType} unknown" )
    ...
    def __eq__ ( self, other ):
        ...
        raise ValueError ( f"comparing a NllEvalType with {other}({type(other)})" )
    ...
    while ra < 0.0:
        ...
        if i > ntrials or a < -10000.0 or ra is None or ( a < 0 and not allowNegative ):
            ...
            if not foundExtra:
                ...
                print(f"mu_hat={mu_hat:.2f}, sigma_mu={sigma_mu:.2f}")
                raise ValueError(
                    f"cannot find an a that is left of the root. last attempt, a={a:.2f}, root = {ra:.2f}."
                )
    i = 0
        ...
        if i > ntrials or ( b < 0 and not allowNegative ):
            ...
            for b in bvalues:
                ...
                if rb < closestr:
                    ...
                    closest = b
            if not foundExtra:
                print(f"cannot find a b that is right of the root (i.e. rootfinder(b) < 0).")
                print(f"closest to zero rootfinder({closest})={closestr}")
                print(f"mu_hat was at {mu_hat:.2f} sigma_mu at {sigma_mu:.2f}")
                raise ValueError()
    return a, b
...
def getChi2(mu,ns,nExpected,nobs,covMatrix,deltas=0.0):
    theory = (nExpected + mu*ns)
    diff = (theory - nobs)    
    covariance = covMatrix + np.diag((mu*ns*deltas)**2)
    Vinv = np.linalg.inv(covariance)
    return ((diff).dot(Vinv)).dot(diff)


def getDeltaChi2(mu,ns,nExpected,nObs,covMatrix,deltas=0.0):

    #First find minima of the chi profile, such that the delta chi2 can then be calculated
    def func_to_solve_deltachi2(mu0):
        return getChi2(mu0, ns, nExpected, nObs, covMatrix, deltas)

    mu_hat = minimize(func_to_solve_deltachi2, x0=0).x
    chi2min = getChi2(mu_hat, ns, nExpected, nObs, covMatrix, deltas)

    
    chi20 = getChi2(mu, ns, nExpected, nObs, covMatrix, deltas)

    return chi20-chi2min

def getULfromChi2(ns,nExpected,nObs,covMatrix,deltas=0.0,mu_max=1000.):

    #First find minima of the chi profile, such that the delta chi2 can then be calculated
    def func_to_solve_deltachi2(mu0):
        return getChi2(mu0, ns, nExpected, nObs, covMatrix, deltas)

    mu_min = minimize(func_to_solve_deltachi2, x0=0).x
    chi2min = getChi2(mu_min, ns, nExpected, nObs, covMatrix, deltas)

    def func_to_solve_95(mu0):
        return getChi2(mu0, ns, nExpected, nObs, covMatrix, deltas) - chi2min - 3.84

    
    mu_UL = brentq(func_to_solve_95, a=mu_max,b=mu_min)
